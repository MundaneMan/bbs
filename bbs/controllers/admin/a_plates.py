...
class PlatesManageHandler(AdminBaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        if category_id != "add":
            ...
            if cur_category:
                ...
                self.redirect("/admin/plates?category_id=add")
                return
        else:
            cur_category = None

        self.render("a_plates.html", categories=root_categories,
    ...
    def post(self):
        ...
        if category_id == "" or category_id == "add":
            new_main_name = self.get_argument("main_name", "")
            # 新分类的内容为空
            if not new_main_name:
                self.redirect("/admin/plates")
                return
            else:
                # 添加新分类
            ...
            if cur_category:
                # 删除所有分类
                main_name = self.get_argument("main_name", "")
                if not main_name:
                    ...
                    r1 = plate_model.update_name_by_id(category_id, {"name": main_name})
                    if not r1:
                    ...
                    for arg_key, arg_value in self.request.arguments.items():
                        if str(arg_key).startswith("sub_"):
                            ...
                            modify_sub_category["name"] = str(arg_value[0])
                            modify_sub_category["parent"] = category_id
                            modify_sub_categories.append(modify_sub_category)
                    sub_categories = plate_model.list_categories_by_cond(
                        {"parent": category_id, "status": "normal"})
                    ...
                    for name in new_sub_categories:
                        res = plate_model.insert_category({"name": name,
                                                           "parent": category_id})
                        if not res:
                    ...
                    for category in modify_sub_categories:
                        _id = category['_id']
                        category.pop('_id')
                        s = plate_model.update_name_by_id(_id, category)
                        if not s:
                            self._message_page(u"<strong style='color:red'>修改分类出错，文章分类'{0}'已存在.</strong>"
                                               .format(category["name"].decode("utf-8")),
                                               category_id=category_id, t_seconds=2)
                            return
            else:
                self.redirect("/admin/plates")
    ...
    def _get_one_level_sub_categories(self, category):
        sub_categories = [plate_model.format_category(c) for c in plate_model.list_categories_by_cond(
            {"status": "normal", "parent": str(category["_id"])})]
        if sub_categories:
    ...
    def _message_page(self, message, category_id="add",t_seconds=1):
        self.write(u"<center>"+message+".</center><script type='text/javascript'>"
                   "function jumpPage(){window.location.href='/admin/plates?category_id="+category_id+"';};"
                   "window.setTimeout('jumpPage();',"+str(t_seconds*1000)+");"
                   "</script>")

urls = [
    (r"/admin/plates/?", PlatesManageHandler),
]