...
def load_operation_log_by_id(str_id, status="normal"):
    ...
    return db.app_bbs_operation_logs.find_one({"_id": obj_id, "status": status})


def list_operation_log_by_cond(cond, sort=None, start=0, limit=30, _is_count=False):
    if _is_count:
        return db.app_bbs_operation_logs.find(cond).count()

    operation_log_cursor = db.app_bbs_operation_logs.find(
...
def insert_operation_log(operation_log_data):
    ...
    return db.app_bbs_operation_logs.insert_one(operation_log_data)
